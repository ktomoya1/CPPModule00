...
bool  PhoneBook::SearchContact(Contact* contacts) {
  int index;
  if (!this->GetInputNumber("Please enter the index of the contact you want to view: ", index))
    return false;
...
void  PhoneBook::DisplayContactDetails(Contact& contact) {
  std::cout << std::setw(16) <<          "index: " << contact.GetIndex()         << std::endl
            << std::setw(16) <<     "first name: " << contact.GetFirstName()     << std::endl
